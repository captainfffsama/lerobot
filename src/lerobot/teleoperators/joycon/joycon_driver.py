...
class JoyconRobotics:
    def __init__(
        ...
        # use to detect joycon disconnect
        self._previous_gyro_data=np.random.rand(3,3).astype(np.float64)
        ...
        # L normal v 2193
        # L normalh 2050
        self.JOY_STICK_V_MAX = 2500
        self.JOY_STICK_V_MIN = 1300
        self.JOY_STICK_H_MAX = 2500
        self.JOY_STICK_H_MIN = 1500

        self.have_run_count = 0
        ...
        self.device_alive_check_thread = threading.Thread(target=self.device_alive_check_loop, daemon=True)
        self.device_alive_check_thread.start()

    def device_alive_check_loop(self):
        while self.running:
            time.sleep(3.0)
            try:
                self.joycon._joycon_device.read(3)
            except HIDException as e:
                logging.error(f"JoyCon disconnected: {e}")
                self.running = False
    ...
    def common_update(self):
        ...
        if joycon_button_down == 1:
            # NOTE: notice pure model return delta xyz
            if self.pure_xz:
                self.position[2] -= 0.001 * self.dof_speed[2]
            else:
                self.position[0] -= (
    ...
    def get_orientation(self):  # euler_rad, euler_deg, quaternion,
        ...
        gyro_data=np.array(self.gyro.gyro_in_rad)
        acc_data=np.array(self.gyro.accel_in_g)

        self.orientation_rad = self.orientation_sensor.update(
            gyro_data.mean(0).tolist(), acc_data.mean(0).tolist()
        )
    ...
    def update(self):
        # self.have_run_count += 1
        # print("have_run_count:", self.have_run_count)
    ...
    def get_control(self, out_format="euler_rad"):
        if not self.running:
            print("\033[31mJoycon disconnected! Please check the connection.\033[0m")
        ...
        x, y, z = self.position
        # print(f"device {self.device_name}:-x:{x}, y:{y}, z:{z}, roll:{roll}, pitch:{pitch}, yaw:{yaw}")
