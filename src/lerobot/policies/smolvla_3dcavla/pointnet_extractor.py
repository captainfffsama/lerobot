...
class PointNetfeat(nn.Module):
    def __init__(self, out_dim, global_feat = True, feature_transform = False, use_MLP = True):
        ...
        self.proj = nn.Linear(1024, out_dim) # project dimention of global features to match LLM dimension
...
if __name__=="__main__":
    ...
    pn = PointNetfeat(2048)
    
    x = torch.rand(64, 3, 4096, dtype=torch.float32)
    print(x.shape)
    y = pn(x)
    print(y[0].shape)