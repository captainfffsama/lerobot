...
from lerobot.common.policies.smolvla_3dcavla.smolvlm_with_expert import SmolVLMWithExpertModel
from lerobot.common.policies.utils import (
    populate_queues,
)
from lerobot.common.policies.smolvla_3dcavla.pointnet_extractor import PointNetfeat
...
class SmolVLAPolicy3d_cavla(PreTrainedPolicy):
    """Wrapper class around VLAFlowMatching model to train and run inference within LeRobot."""

    config_class = SmolVLAConfig3d_cavla
    name = "smolvla"

    def __init__(
        self,
        config: SmolVLAConfig3d_cavla,
    ...
    def prepare_images(self, batch):
        ...
        #observation.images.cam_left摄像头出错已不可使用
        # Preprocess image features present in the batch
        for key in present_img_keys:
            if key=='observation.images.cam_left':
                # print('observation.images.cam_left is not used')
                continue
    ...
    def prepare_points(self,batch):
        point_feature1 = batch['observation.points.cam_high'].squeeze()
        point_feature2 = batch['observation.points.cam_right'].squeeze()
        
        # points = torch.cat([point_feature1,point_feature2],dim=1)
        point_feature1 = point_feature1.transpose(1, 2)
        point_feature2 = point_feature2.transpose(1, 2)
        
        return (point_feature1,point_feature2)
...
class VLAFlowMatching(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.point_proj1 = PointNetfeat(self.vlm_with_expert.config.text_config.hidden_size)
        self.point_proj2 = PointNetfeat(self.vlm_with_expert.config.text_config.hidden_size)
    ...
    def embed_prefix(
        ...
        #点云
        point_emb1 = self.point_proj1(pointcloud[0])[0]
        point_emb2 = self.point_proj2(pointcloud[1])[0]
        point_emb = torch.stack([point_emb1, point_emb2], dim=1) 
